...
class Discriminator(nn.Module):
    ...
    def update(self, expert_batch, policy_batch, optim_dis, lambda_gp, label_smoothing, smooth):
        ...
        loss_discriminator = compute_discriminator_loss(exp_scores, gen_scores, 
                                                        lambda_gp, grad_penalty, 
                                                        label_smoothing=label_smoothing, smooth=smooth) #ggf. WGAN-Loss direkt
        
        optim_dis.zero_grad()
        loss_discriminator.backward()
        optim_dis.step()

        return (loss_discriminator.item(), grad_penalty.item(), exp_scores.mean().item(), gen_scores.mean().item())