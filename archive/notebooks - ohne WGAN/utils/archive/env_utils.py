...
def get_env_data(global_state, frame):
    ...
    for row in data_dict.keys():
        ...
        vx, vy = agent[3], agent[4] #das ist Falsch
...
def env_pred_to_tensor(df, type, device="cpu"):
...
def env_prey_to_tensor(df, type, device="cpu"):
    feature_map = {'state':  ['dx', 'dy', 'rel_v_x', 'rel_v_y'], 'action': ['theta']}
...
def get_state_action_tensors(records):
    ...
    env_pred_state  = env_pred_to_tensor(df_pred, "state", device)
    env_pred_action = env_pred_to_tensor(df_pred, "action", device)
    ...
    env_prey_state  = env_prey_to_tensor(df_prey, "state", device)
    env_prey_action = env_prey_to_tensor(df_prey, "action", device)
...
def parallel_get_rollouts(pred_count, prey_count, pred_policy, prey_policy, num_frames=10, render=False, seed=None, device="cpu"):
    ...
    try:    
        ...
        for frame_idx in range(num_frames):
            ...
            global_state = env.state()
            env_data = get_env_data(global_state, frame_idx)
            env_pred_t, env_prey_t = get_state_action_tensors(env_data)

            pred_states = env_pred_t[..., :4]
            prey_states = env_prey_t[..., :4]

            con_pred = pred_policy.forward_pred(pred_states)
            pred_disc = continuous_to_discrete(con_pred, env.unwrapped.action_count).item()
            con_prey = prey_policy.forward_prey(prey_states)
            prey_disc_ls = continuous_to_discrete(con_prey, env.unwrapped.action_count).flatten().tolist()

            pred_tensors.append(torch.cat([pred_states, con_pred.unsqueeze(-1).unsqueeze(-1).expand(-1,-1,pred_states.size(2),-1)], dim=-1))
            prey_tensors.append(torch.cat([prey_states, con_prey.unsqueeze(-1).unsqueeze(-1).expand(-1,-1,prey_states.size(2),-1)], dim=-1))

            action_dict = {'predator_0': pred_disc}
            for i, agent_name in enumerate(sorted([a for a in env.agents if a.startswith("prey")])):
                action_dict[agent_name] = prey_disc_ls[i]
        ...
        pred_tensor = torch.stack(pred_tensors, dim=0).squeeze(1)
...
def generate_trajectories(buffer, pred_count, prey_count, pred_policy, prey_policy, render=False, num_frames=9, num_generative_episodes=10):
    seeds = np.random.randint(0, 1000, size=num_generative_episodes).tolist()
