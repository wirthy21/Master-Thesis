...
class GeneratorPolicy(nn.Module):
    ...
    def update(self, role, network, env,
        ...
        orig_theta = nn.utils.parameters_to_vector(module.parameters()).detach().clone()
        dim = orig_theta.numel()
        lr = lr_pred_policy if role == "predator" else lr_prey_policy

        # Evolution Strategy Hauptschleife
        for generation in range(num_generations):
            ...
            for i in range(num_perturbations):
                eps = torch.randn(dim, device=orig_theta.device) * sigma
                epsilons.append(eps)

                # Positive Perturbation
                nn.utils.vector_to_parameters(orig_theta + eps, module.parameters())
                pt_pos, yt_pos = get_rollouts(env, pred_policy, prey_policy, num_frames=9)
                r_pos, _ = discriminator_reward(pt_pos, yt_pos, pred_discriminator, prey_discriminator)
                pos_rewards.append(r_pos.mean().item())

                # Negative Perturbation
                nn.utils.vector_to_parameters(orig_theta - eps, module.parameters())
                pt_neg, yt_neg = get_rollouts(env, pred_policy, prey_policy, num_frames=9)
                r_neg, _ = discriminator_reward(pt_neg, yt_neg, pred_discriminator, prey_discriminator)
                neg_rewards.append(r_neg.mean().item())

                nn.utils.vector_to_parameters(orig_theta, module.parameters())
                rewards_diff.append((r_pos - r_neg).detach())

            # Rewards normalisieren und Gradient schätzen
            rewards_norm = normalize(rewards_diff)
            theta_est = gradient_estimate(orig_theta, rewards_norm, dim,
                                          epsilons, sigma, lr, num_perturbations)

            diff_stack = torch.stack(rewards_diff)
            grad_norm = (theta_est - orig_theta).norm().item()
            metrics.append({
                "generation": generation,
                "avg_reward_diff": diff_stack.mean().item(),
                "best_positive_reward": max(pos_rewards),
                "sigma": sigma,
                "lr": lr,
                "grad_norm": grad_norm})

            # Update Parameter und Hyperparameter anpassen
            nn.utils.vector_to_parameters(theta_est, module.parameters())
            orig_theta = theta_est.detach().clone()
            sigma *= gamma
            lr *= gamma
