...
from custom_marl_aquarium.env.utils import (
    Torus,
...
from custom_marl_aquarium.env.view import View
...
class raw_env(ParallelEnv[str, Box, Discrete | None]):  # pylint: disable=C0103
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[Dict] = None):
        ...
        if options:
            ...
            prey_positions = positions[1:]
            self.predators = self.create_predators(positions=pred_positions)
            self.prey = self.create_prey(positions=prey_positions)
        else:
            # Random-Reset
            self.predators = self.create_predators()
            self.prey = self.create_prey()
    ...
    def create_costumized_prey(self, prey_positions):
        x = prey_positions[0]
        y = prey_positions[1]

        scaled_xs = scale(x, 0, 1, 0, self.width)
        scaled_ys = scale(y, 0, 1, 0, self.height)

        costum_inital_pos = Vector(scaled_xs, scaled_ys)
        costum_inital_vel = Vector(0, -1)
        costum_inital_acc = Vector(0, 0)
    ...
    def create_costumized_predator(self, predator_positions):
        x = predator_positions[0]
        y = predator_positions[1]
    ...
    def create_predators(self, positions=None):
        """Creates a list of predators"""
        if positions is not None:
            return [self.create_costumized_predator(positions) for _ in range(self.predator_count)]
        else:
            return [self.create_random_predator() for _ in range(self.predator_count)]

    def create_prey(self, positions=None):
        """Creates a list of prey"""
        if positions is not None:
            return [self.create_costumized_prey(positions[i]) for i in range(self.prey_count)]
        else:
    ...
    def check_borders_continuous(self, animal: Entity):
    ...
    def check_borders_fix(self, animal: Entity):
        angle = animal.orientation_angle
        delta = random.uniform(-math.radians(90), math.radians(90)) #offset if 90°

        bounced = False
        if animal.position.x >= self.width:
            animal.position.x = self.width
            new_angle = math.pi - angle + delta
            bounced = True
        elif animal.position.x <= 0:
            animal.position.x = 0
            new_angle = math.pi - angle + delta
        ...
        if bounced:
            new_angle %= 2 * math.pi
            animal.change_orientation(new_angle)

    def apply_wall_repulsion(self, animal, d=50, k=5):
        force = Vector(0, 0)
        # left wall
        if animal.position.x < d:
            strength = k * (1 - animal.position.x / d)
            force.x += strength
        # right wall
        dist_right = self.width - animal.position.x
        if dist_right < d:
            strength = k * (1 - dist_right / d)
            force.x -= strength
        # bottom wall
        if animal.position.y < d:
            strength = k * (1 - animal.position.y / d)
            force.y += strength
        # top wall
        dist_top = self.height - animal.position.y
        if dist_top < d:
            strength = k * (1 - dist_top / d)
            force.y -= strength
        # apply the computed repulsion
        animal.apply_force(force)

        animal.position.x = max(0, min(animal.position.x, self.width))
        animal.position.y = max(0, min(animal.position.y, self.height))
    ...
    def update_prey(self, prey: Prey, predators: List[Predator], desired_velocity: Vector):
        ...
        if self.use_walls:
            self.apply_wall_repulsion(prey)
        else:
            self.check_borders_continuous(prey)
    ...
    def update_predator(self, predator: Predator, desired_velocity: Vector):
        ...
        if self.use_walls:
            self.apply_wall_repulsion(predator)
        else:
            self.check_borders_continuous(predator)
