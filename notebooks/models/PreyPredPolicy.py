...
class PreyPolicy(nn.Module):
    ...
    def forward(self, states):
        ...
        return prey_action_per_prey, mu_log, sigma_log, weights_log
...
#################################
##### PredInteractionPolicy #####
#################################


class PredInteractionPolicy(nn.Module):
    def __init__(self):
        super(PredInteractionPolicy, self).__init__()

        self.pred_pairwise = PredatorInteraction(features=4)

    def forward(self, states):
        mu_pred, sigma_pred = self.pred_pairwise(states)       # mu=32, simga=32
        ...
        return pred_action_flat, mu_pred, sigma_pred
