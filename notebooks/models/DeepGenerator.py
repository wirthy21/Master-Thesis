...
class Attention(nn.Module):
    def __init__(self):
        ...
        self.fc3 = nn.Linear(128, 64)
        self.fc4 = nn.Linear(64, 32)
        self.f5_prey = nn.Linear(32, 1)  # logits for prey-neighbor weights
        self.f5_pred = nn.Linear(32, 1)  # logits for predator weight(s)
...
class GeneratorPolicy(nn.Module):
    ...
    def forward_prey(self, states):
        ...
        h_prey = F.relu(self.attention.fc3(h_prey))
        h_prey = F.relu(self.attention.fc4(h_prey))
        w_prey_flat = self.attention.f5_prey(h_prey)
        ...
        h_pred = F.relu(self.attention.fc3(h_pred))
        h_pred = F.relu(self.attention.fc4(h_pred))
        w_pred_flat = self.attention.f5_pred(h_pred)
